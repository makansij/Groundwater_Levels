...
for (i in a){
  for (j in seq(1,n)){
    ...
    perm_array = c(perm_array,list(Y))
  }
}
...
histo_flux <- function(q_matrix){
  ...
  for (i in 1:num_rows){    
    ...
    hist_flux <- hist(fluxes,breaks=10,main=paste("Conditional Flux HISTOGRAM OF L/2 FOR a = ",a[i],sep="",collapse=NULL))    
    dev.off()
  }
}
...
for (i in 1:length(a)){
  ...
  print('now starting a value of ')
  print(a[[i]])
  baseline = perm_matrix[i,1][[1]]  # each row in field_matrix was generated by an "a" value
  a_tmp = a[[i]]
  L = a_tmp*I_y
  dx = L/100
  x <- seq(0,L,by=dx) # simulation grid, as provided in Question 1 of homework  
  grid <- expand.grid(x=x,y=1)
  meas = measurements(baseline,L)  
  vg.exp <- vgm(psill=1.5,model='Exp',range = 15)
  
  # conditional simulations (50 of them)
  cond.fields <- krige(formula=z~1, locations=~x+y, data=meas, newdata=grid, model=vg.exp, nsim=50, beta=mean(meas$z),nmax=25)
  for (j in 3:length(cond.fields)){
    field = cond.fields[[j]]
    H <- Hfield(k=exp(field), h0=70, hl=50, dx=dx, dy=1, oneD=TRUE)    # solves the flow equation 
    # this doesn't work for a>2.5
    field_array = c(field_array,list(H))  #Collect field arrays 
  }
  print('finished generating the  field_array for "a" value: ')
  print(a[[i]])
  plot_name = paste0("Baseline_L=",toString(a_tmp),".jpg",sep="")
  jpeg(plot_name)
  plot(meas$x,meas$z,xlab="x distance",ylab="Z values",ylim=c(-5,5),xlim=c(0,L),main=plot_name,type='p',pch=15)
  kout <- krige(formula=z~1, locations=~x+y, data=meas, newdata=grid, model=vg.exp, beta=mean(meas$z),nmax=25)
  lines(x,cond.fields[[10]],lty=1,col='black',lwd=2.5) # baseline 
  lines(x,cond.fields[[4]],lty=1,col='blue',lwd=1.0) # a few simulations 
  lines(x,cond.fields[[5]],lty=1,col='blue',lwd=1.0)
  lines(x,cond.fields[[6]],lty=1,col='blue',lwd=1.0)
  lines(x,cond.fields[[7]],lty=1,col='blue',lwd=1.0)
  lines(x,kout$var1.var,lty=3,col='red',lwd=2.5)   # kriging variance 
  lines(x,rep(mean(kout$var1.pred),length(x)),lty=3,col='green',lwd=2.5)   # kriging mean
  second_stdv = 2*sqrt(var(baseline))
  lines(x,rep(second_stdv,length(x)),lty=3,col='black',lwd=2.0)  # 2 standard deviations of the population
  lines(x,-rep(second_stdv,length(x)),lty=3,col='black',lwd=2.0)  # 2 standard deviations of the population
  lines(x,rep(1.5,length(x)),lty=3,col='orange',lwd=2.0)  # population variance 
  legend('bottomright',c('Baseline','Simulations','Kriging Variance','Kriging mean','2 Standard Deviations','Population Variance'),lty=c(1,1,3,3,3,3,3),col=c('black','blue','red','green','black','orange'))
  dev.off()  
  plot_name=""
}
...
## Question 3 part c
# here we assume that cond.fields is already created as it was in Question 3 part b 
...
measurements <- function(baseline_field,L){
  ...
  x_locations = c(10,20,30,32,34,36,38,40,50,55,60,70,80,90,100)
