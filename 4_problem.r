...
fccz0 <- function(z0_min,z0_max,z,r,pdf){
  ...
  result = integrate(fccz,z0_min,z0_max,z,r,pdf,subdivisions = 50, stop.on.error = TRUE)
...
# range of r values:
r_vect <- c(1.0) #c(0.25,0.5,1.0,2.0,5.0,10.0)
...
for (r in r_vect){
  ...
  for (z in z_vect){
    #P = 0.7
    part1 <- fccz0(2,2.5,z,r,0.7)
    #P = 0.3
    part2 <- fccz0(2.5,3.0,z,r,0.3)
    
    result <- as.numeric(part1[1]) + as.numeric(part2[1])  
    integral <- c(integral,result)
  }
  plot(z_vect, integral ,main=paste("Conditional pdf of r=",toString(r)),xlab='z',ylab='pdf') # show data locations
}
